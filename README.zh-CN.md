...
- **多种输入源** — 内联文本、`.md`、`.txt`、`.pdf` 文件，或 URL 网页
- **自动导出** — `.tsv`（Anki 直接导入）、`.apkg`（便携牌组）或通过 AnkiConnect 直推到 Anki
...
```
从 https://zh.wikipedia.org/wiki/Dijkstra算法 生成 Anki 闪卡
```
...
# AnkiConnect 直推（需要 Anki 运行中）
anki-export cards.txt --ankiconnect -d "我的牌组"
```
